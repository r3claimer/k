...
static irqreturn_t ads1015_trigger_handler(int irq, void *p)
{
	...
	printk("lqb func = %s new result = %d\n",__func__,res);
	if (ret < 0) {
...
static int adc_to_abs(int val,enum adc_val a)
{
	int abs_val;
	switch(a){
	case left_val:
		...
		if(val < 885)	val = 885;
		  abs_val = 255 - (1080-val)*255/195;
		break;
	case right_val:
		if(val>1200)	val = 1200;	
		if(val<950)		val = 950;
		  abs_val =255 - (1200-val)*255/250;
		break;
	}
...
static void adc_banji_poll(struct input_dev *input)
{
	...
	struct ads1015_data *data = input_get_drvdata(input);
		...
		if (ret)
		{
			printk("lqb iio_device_claim_direct_mode error!\n");
			return;
		}
...
//			printk("lqb func = %s new result[%d] = %d\n",__func__,i,val[i]);
		}
...
//			printk("lqb func = %s new result[%d] = %d\n",__func__,i,val[0]);
		...
		if(val[2]<948 &&val[2]>838)
		{
			val[0] = 888;		
			input_event(input, EV_KEY, BTN_TL2, 0);			
			input_report_abs(data->input, ABS_BRAKE,  0);
			data->last_data[2] = val[2];
		}else if(val[2]> 948)
		{			
			input_event(input, EV_KEY, BTN_TL2, 1);
			
			if(val[2] !=data->last_data[2])
			{
				val[2] = adc_to_abs(val[2],left_val);
				//input_report_abs(data->input, ABS_RY,  val[2]*2);	
				input_report_abs(data->input, ABS_BRAKE,  val[2]);
				data->last_data[2] = val[2];
			}
		}

		if(val[3]<970 &&val[3]>890){
			val[3] = 950;			
			input_event(input, EV_KEY, BTN_TR2, 0);		
			input_report_abs(data->input, ABS_GAS,  0);
			data->last_data[3] = val[3];
		}else if(val[3]>1000)
		{			
			input_event(input, EV_KEY, BTN_TR2, 1);
			if(val[3] !=data->last_data[3])
			{
			
				val[3] = adc_to_abs(val[3],right_val);
//				input_report_abs(data->input, ABS_RX,  val[3]*2);
				input_report_abs(data->input, ABS_GAS,  val[3]);
...
//		__set_bit(BTN_JOYSTICK,input->keybit);

//		set_bit(ABS_RX, input->absbit);		
//		set_bit(ABS_RY, input->absbit);		
//		set_bit(KEY_PAGEUP, input->absbit);
		set_bit(ABS_BRAKE, input->absbit); 
		set_bit(ABS_GAS, input->absbit);
//		input_set_abs_params(input, ABS_RY, 0, 2400, 0, 15);
//		input_set_abs_params(input, ABS_RX, 0, 2400, 0, 15);
...
//			1 << ADS1015_CFG_COMP_LAT_SHIFT;
		...
		if (ret)
		{
				printk("lqb devm_request_threaded_irq error!!!!!!\n");
			return ret;
		}
	}
	...
	if (ret)
	{
	printk("lqb ads1015_set_conv_mode error!!!!!!\n");
		return ret;
		}
	data->conv_invalid = true;
	...
	if (ret)
	{
		printk("lqb pm_runtime_set_active error!!!!!!\n");
	return ret;
	...
	printk("lqb available_scan_masks = %d\n",indio_dev->available_scan_masks);
	return 0;
}
