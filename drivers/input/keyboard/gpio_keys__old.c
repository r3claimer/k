...
//	struct gpio_keys_platform_data *p_data;
	struct platform_device *dev;
	struct gpio_keys_drvdata *st;
	char *token;
	int err;
	...
	DBG("peng_store lqb buf:%s count:%d\n",buf,count);
//	DBG();
	strcpy(str,buf);
	
	dev = to_platform_device(pdev);
	DBG("peng_store str =  %s\n",str);
	st = dev_get_drvdata(&dev->dev);
//	st = platform_get_drvdata(const struct platform_device * pdev)
	if(buf == NULL) return -1;
	if(count < 3)
	{
		DBG("peng_store buf == 1,get_raw_data");
		get_raw_data(st);	
	}
	else{
		//token = strtok();strsep
		...
		while( token && cou < 4){
			...
			DBG("peng_store %d Conversion result is %d\n",cou,val[cou]);
		    if (err) {
