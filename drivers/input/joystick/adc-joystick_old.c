...
static void adc_keys_poll(struct input_dev *input)
{
	...
	{
		iio_read_channel_processed(&joy->chans[i], &value);
		//printk("lqb joy->chans[%d] = %d\n",i,value);
		input_report_abs(joy->input, joy->axes[i].code, value);
	}
	input_sync(joy->input);
//	ret = iio_read_channel_processed(&st->chans[0], &value1);
//	ret = iio_read_channel_processed(&st->chans[1], &value2);
//	printk("st->chans[0] = %d,value2 = %d\n",value1,value2);
...
//	const u16 *data_u16;
//	bool sign;
//	printk("lqb11111 adc_joystick_handle\n");
...
//		printk("lqb222222 adc_joystick_handle\n");
//
//		switch (bytes) {
//		case 1:
...
static int adc_joystick_probe(struct platform_device *pdev)
{
	...
	int bits;
	int i;
		printk("lqb adc_joystick_probe successful!!!\n");
	joy = devm_kzalloc(dev, sizeof(*joy), GFP_KERNEL);
	if (!joy)
		return -ENOMEM;

	joy->chans = devm_iio_channel_get_all(dev);
	printk("lqb adc_joystick_probe11111111111 successful!!!\n");
	if (IS_ERR(joy->chans)) {
...
//	input->close = adc_joystick_close;
	input_set_poll_interval(input, 100);
...
//		return error;
//	}
	printk("lqb adc_joystick_probe2222222222 successful!!!\n");
