...
//	gpio_direction_output(st->adc_hall_gpios, 0);
	...
	DBG("lqbjoy read adc_to_abs[x_val] %d,value = %d\n",adc_to_abs(value),value);
	if(st->data[0] == 127) st->data[0] = 128;
//	 printk("lqbjoy adc_joystick_poll: read joystick x_val = %d \n",value);
...
//	 printk("lqbjoy adc_joystick_poll: read joystick y_val = %d \n",value);
...
//		input_report_key(dev->input, BTN_TR2,  btn_date[1]);
		...
		input_sync(st->input);
		state = 0;
		 DBG("lqbjoy adc_joystick_poll: input_report_abs [x=%d,y=%d\n",abs_date[0],abs_date[1]);
	}
...
//	struct input_polled_dev *poll_dev;
	...
	//joystick y
	DBG("adc_joystick_probe joystick_y \n");
